...
public class Base64 {
	 ...
	 * that data encoded this way is <em>not</em> officially valid Base64,
	 * or at the very least should not be called Base64 without also
	 * specifying that is was encoded using the URL- and Filename-safe
	 * dialect.
	 */
	...
	/**
	 * Translates a Base64 value to either its 6-bit reconstruction value or
	 ...
	 * there is no guarantee as to which one will be picked.
	 */
	 ...
	 * options specified. It's possible, though silly, to specify ORDERED
	 * and URL_SAFE in which case one of them will be picked, though there
	 ...
	 * and returns a four-byte array in Base64 notation. The actual number
	 * of significant bytes in your array is given by
	 * <var>numSigBytes</var>. The array <var>threeBytes</var> needs only be
	 * as big as
	 ...
	 * it to the <code>encoded</code> ByteBuffer. This is an experimental
	 * feature. Currently it does not pass along any options (such as
	...
	/**
	 * Performs Base64 encoding on the <code>raw</code> ByteBuffer, writing
	 * it to the <code>encoded</code> CharBuffer. This is an experimental
	 * feature. Currently it does not pass along any options (such as
	 * {@link #DO_BREAK_LINES} or {@link #GZIP}.
	 *
	 * @param raw input buffer
	 ...
	 * to v2.3!</b> In earlier versions, it just returned a null value, but
	 ...
	 * will throw an java.io.IOException. <b>This is new to v2.3!</b> In
	 * earlier versions, it just returned a null value, but in retrospect
	 ...
	 * java.io.IOException. <b>This is new to v2.3!</b> In earlier versions,
	 * it just returned a null value, but in retrospect that's a pretty poor
	 * way to handle it.
	 * </p>
	 *
	 *
	 ...
	 * @return The Base64-encoded data as a String
	 * @throws NullPointerException if source array is null
	 * @throws IllegalArgumentException if source array, offset, or length
	 * are invalid
	 * @since 1.4
	 */
	 ...
	 * byte array instead of instantiating a String. This is more efficient
	 * if you're working with I/O streams and have large data sets to
	 * encode.
	 *
	 *
	 * @param source The data to convert
	 ...
	 * invalid or there is not enough room in the array.
	 * @since 1.3
	 */
	 ...
	 * not generally a recommended method, although it is used internally as
	 ...
	 * @throws ClassNotFoundException if the decoded object is of a class
	 * that cannot be found by the JVM
	 * @since 1.5
	 */
	...
	/**
	 * Attempts to decode Base64 data and deserialize a Java Object within.
	 * Returns <tt>null</tt> if there was an error. If <tt>loader</tt> is
	 ...
	 * java.io.IOException. <b>This is new to v2.3!</b> In earlier versions,
	 * it just returned false, but in retrospect that's a pretty poor way to
	 * handle it.
	 * </p>
	 *
	...
	public static class InputStream extends java.io.FilterInputStream {
		...
		/**
		 * Constructs a {@link Base64.InputStream} in either ENCODE or
		 * DECODE mode.
		 * <p>
		 * Valid options:
		 *
		...
		/**
		 * Calls {@link #read()} repeatedly until the end of stream is
		 * reached or <var>len</var> bytes are read. Returns number of
		 * bytes read into array or -1 if end of stream is encountered.
		 *
		 * @param dest array to hold values
		 * @param off offset for array
		 * @param len max number of bytes to read into array
		 * @return bytes read into array or -1 if end of stream is
		 * encountered.
		 * @since 1.3
		 */
		@Override
	...
	public static class OutputStream extends java.io.FilterOutputStream {
		...
		/**
		 * Writes the byte to the output stream after converting to/from
		 * Base64 notation. When encoding, bytes are buffered three at a
		 * time before the output stream actually gets a write() call.
		 ...
		 * buffer without closing the stream.
		 *
		 ...
		 * to embed a piece of base64-encoded data in a stream.
		 *
		...
		/**
		 * Resumes encoding of the stream. May be helpful if you need to
		 * embed a piece of base64-encoded data in a stream.
		 *
		 * @since 1.5.1
		 */
